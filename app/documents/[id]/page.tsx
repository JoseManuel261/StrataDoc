...
import { ArrowLeft, CheckCircle2, Loader2, Link2, History } from 'lucide-react'
...
export default function DocumentEditorPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        if (docData.content) setCurrentContent(docData.content as Record<string, unknown>)
      } catch {
  ...
  function handleRestoreVersion(content: Record<string, unknown>) {
      ...
      <div style={{
        ...
        <input
          ...
          style={{
            ...
            color: 'var(--text)', fontFamily: 'Fenix, serif', fontSize: '1rem', letterSpacing: '-0.01em',
          }}
        />
        ...
        <select
          ...
          style={{ background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', borderRadius: '0.375rem', padding: '0.25rem 0.5rem', fontSize: '0.72rem', fontFamily: 'DM Mono, monospace', outline: 'none', cursor: 'pointer', flexShrink: 0 }}
        >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem', flexShrink: 0 }}>
          {doc.project_id && (
          <span title="Vinculado a proyecto de Strata" style={{ display: 'flex', alignItems: 'center' }}>
          ...
          <select
            ...
            style={{ background: 'var(--surface2)', border: '1px solid var(--border)', color: doc.project_id ? 'var(--text)' : 'var(--text-dim)', borderRadius: '0.375rem', padding: '0.25rem 0.5rem', fontSize: '0.72rem', fontFamily: 'DM Mono, monospace', outline: 'none', cursor: 'pointer', maxWidth: '150px' }}
          >
        ...
        <button
          ...
          style={{
            ...
            border: showVersions ? '1px solid var(--blue)' : '1px solid var(--border)',
            background: showVersions ? 'rgba(96,165,250,0.1)' : 'transparent',
            color: showVersions ? 'var(--blue)' : 'var(--text-muted)',
            cursor: 'pointer', fontFamily: 'Syne, sans-serif', fontSize: '0.72rem',
        ...
        {/* Exportar */}
        <ExportMenu documentId={id} documentTitle={title} />
